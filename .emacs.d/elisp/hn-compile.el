...
(defun hn-tags ()
  ...
  (if (not (equal (hn-find-vc-root) nil))
	  (progn
		...
		(setq old-path default-directory)
		(setq default-directory (hn-find-vc-root))
		(shell-command "ctags -e --recurse=yes --exclude=mootools*.js --exclude=dygraph*.js --exclude=jx*.js --exclude=*.css --exclude=venv --exclude=node_modules --exclude=dist --exclude=tmp")
